<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-2xl font-bold text-gray-800 tracking-tight">{{ __('Edit Transfer Saldo') }}</h2>
                <p class="text-sm text-gray-500 mt-1">Perbarui data perpindahan dana</p>
            </div>
        </div>
    </x-slot>
                ...
                <form action="{{ route('transfers.update', $transfer) }}" method="POST">
                    ...
                    @method('PUT') <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            ...
                            <select name="from_wallet_id"
                                ...
                                @foreach ($wallets as $wallet)
                                    <option value="{{ $wallet->id }}"
                                        {{ old('from_wallet_id', $transfer->from_wallet_id) == $wallet->id ? 'selected' : '' }}>
                        ...
                        <div>
                            ...
                            <select name="to_wallet_id"
                                ...
                                @foreach ($wallets as $wallet)
                                    <option value="{{ $wallet->id }}"
                                        {{ old('to_wallet_id', $transfer->to_wallet_id) == $wallet->id ? 'selected' : '' }}>
                                        {{ $wallet->name }} (Saldo: Rp
                                        {{ number_format($wallet->balance, 0, ',', '.') }})
                                    </option>
                                @endforeach
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" name="amount" min="1"
                                    value="{{ old('amount', round($transfer->amount)) }}" placeholder="0"
                        ...
                        <div>
                            ...
                            <input type="date" name="date" value="{{ old('date', $transfer->date) }}"
                                class="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:border-blue-400 focus:ring-2 focus:ring-blue-200 outline-none"
                                required>
                        </div>
                    </div>

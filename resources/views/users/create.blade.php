<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="text-2xl font-bold text-gray-800 tracking-tight">{{ __('Tambah Pengguna Baru') }}</h2>
        </div>
    </x-slot>
                ...
                <form action="{{ route('users.store') }}" method="POST">
                    ...
                    <div class="mb-6">
                        ...
                        <select name="role"
                            class="w-full px-4 py-2.5 rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:border-blue-400 focus:ring-2 focus:ring-blue-200 outline-none"
                            required>
                            <option value="user" {{ old('role') == 'user' ? 'selected' : '' }}>User (Anggota Keluarga)
                            </option>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8 border-t border-gray-100 pt-6 mt-6">
                        <div>
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Password Login</label>
